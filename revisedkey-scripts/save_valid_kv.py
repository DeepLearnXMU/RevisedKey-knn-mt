    ...
    for subset in args.valid_subset.split(","):
        ...
        with torch.no_grad():
            ...
            for i, sample in enumerate(progress):
                ...
                # logger.info(f'{dstore_idx}:{reduce_size + dstore_idx} = {target.unsqueeze(-1).cpu().numpy().astype(np.int)}')
                # raise ValueError

                dstore_keys.append(features.detach().cpu().numpy().astype(np.float16))
                dstore_vals.append(target.unsqueeze(-1).cpu().numpy().astype(np.int))
        ...
        # print([key.shape for key in dstore_keys])
        # print([val.shape for val in dstore_vals])

        dstore_keys = np.concatenate(dstore_keys)
        dstore_vals = np.concatenate(dstore_vals)

        np.save(args.dstore_mmap + '/keys', dstore_keys)
        np.save(args.dstore_mmap + '/vals', dstore_vals)
