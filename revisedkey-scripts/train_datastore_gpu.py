...
if args.dstore_size == 0:
    with open(args.dstore_mmap + '/size', 'r') as f:
        args.dstore_size = int(f.read())

if args.dstore_fp16:
    logger.info(f'load dstore fp16 {args.dstore_size} {args.dimension}')
...
logger.info(random_sample[:10])
logger.info(keys[random_sample][:10])
gpu_index.train(keys[random_sample].astype(np.float32))
