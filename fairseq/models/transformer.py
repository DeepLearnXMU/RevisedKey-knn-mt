...
from fairseq.modules.knn_adapter import Adapter
...
class TransformerModel(FairseqEncoderDecoderModel):
    ...
    def add_args(parser):
        ...
        parser.add_argument("--retrieve-adapter", default=False, action='store_true')
        parser.add_argument("--use-retrieve-adapter", default=None, type=str)
        parser.add_argument("--adapter-ffn-scale", default=4, type=int)
...
class TransformerDecoder(FairseqIncrementalDecoder):
    ...
    def __init__(self, args, dictionary, embed_tokens, no_encoder_attn=False):
        ...
        if hasattr(args, 'retrieve_adapter') and args.retrieve_adapter:
            self.retrieve_adapter = Adapter(args.decoder_embed_dim, args.adapter_ffn_scale)
            self.retrieve_adapter.load_state_dict(torch.load(args.retrieve_adapter))

        self.use_retrieve_adapter = args.use_retrieve_adapter
    ...
    def forward(
        ...
        if self.use_knn_datastore:
            ...
            if self.use_retrieve_adapter:
                retrieve_hidden = self.retrieve_adapter(last_hidden)
            else:
                retrieve_hidden = last_hidden
                
            knn_search_result = self.knn_datastore.retrieve(retrieve_hidden)
...
@register_model_architecture("transformer", "transformer_domain_de_en")
def transformer_domain_de_en(args):
    args.encoder_embed_dim = getattr(args, "encoder_embed_dim", 512)
    args.encoder_ffn_embed_dim = getattr(args, "encoder_ffn_embed_dim", 1024)
    args.encoder_attention_heads = getattr(args, "encoder_attention_heads", 4)
    args.encoder_layers = getattr(args, "encoder_layers", 6)
